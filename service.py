...
url_payload = { "freshen_url": "https://api.trade.gov/v2/ntto_spending_data/freshen.json?api_key="}
...
def handler(event, context):
  ...
  for obj in bucket.objects.all():
    ...
    if key == 'spending_data.xlsx':
      ...
      try:
        response = s3.Object('ntto-spending-data-json', 'entries.json').put(Body=json.dumps(entries), ContentType='application/json', ACL='public-read')
